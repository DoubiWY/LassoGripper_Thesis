% Instrument mechanics and implementation (formerly Chapter8.tex)
...
% Chapter summary moved to main.tex as a consolidated group summary

\chapter{Mechanical Implementation of the Handheld Instrument} % Main chapter title
\label{Chapter8}
\begin{chaptersummary}
This chapter describes the mechanical and electromechanical implementation of the handheld tendon-driven instrument. It covers actuator selection and arrangement, cable routing and guidance, paired-cable reels and winding mechanisms, tip design and quick-swap interfaces, input module architecture, and packaging considerations for preserving tension, minimizing hysteresis, and reducing distal mass.
\end{chaptersummary}
\section{Mechanism and Design}
\subsection{Output Motors}
A key challenge for handheld cable-driven devices is selecting actuators that combine low mass with sufficient torque. The chosen output actuator, the Feetech STS3032 smart servo, offers a favorable performance-to-weight ratio and built-in feedback useful for closed-loop control. It weighs approximately 9 g and provides a stall torque of about 4.5 kg·cm, enabling reliable tensioning through small-diameter pulleys.

The STS3032's serial-bus communication simplifies wiring and supports daisy-chaining, which reduces the number of conductors running through the handle and aids miniaturization. As a smart servo it reports position, speed, and load, allowing the controller to perform basic load estimation and to implement tension-aware control strategies that improve positional accuracy and protect against overloading.
...
\label{sec:tip_fea}
To validate the structural integrity of the tip assembly, finite element analysis (FEA) simulations were conducted on the two primary load-bearing components: the jaw (gripper) and the middle base. Both models used Aluminum Alloy 6061 as the material. A mesh size of 0.2\,mm was adopted and the pivot shaft was modeled as a fixed support. Cable contact forces were applied as distributed loads over a circular area of 1.2\,mm diameter to represent the cable-guide contact.

\paragraph{Jaw (gripper) FEA}
Model setup: mesh size 0.2\,mm, material Al\,6061, pivot fixed. A 6\,N load was applied over a 1.2\,mm diameter circular contact area at the cable interface. Results show a maximum von Mises stress of 12.4\,MPa (see Fig.~\ref{fig:jaw_fea}). This stress level is well below the typical yield strength of 6061 aluminum ($\approx$55.1\,MPa), indicating a large safety margin for the jaw under the applied loading.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\linewidth]{Figures/Jaw FEA.png}
    \caption{Jaw FEA: von Mises stress distribution under a 6\,N cable load}
    \label{fig:jaw_fea}
\end{figure}

\paragraph{Middle base FEA}
Model setup: mesh size 0.2\,mm, material Al\,6061, pivot fixed. A 6.5\,N load was applied over a 1.2\,mm diameter circular contact area at the cable interface. In addition, the transmitted loads from a pair of jaws (two 6\,N forces) were applied to the upper pivot to represent the combined loading path. The analysis yields a maximum von Mises stress of 9.1\,MPa (see Fig.~\ref{fig:mid_fea}), also well below the material yield strength.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\linewidth]{Figures/Mid FEA.png}
    \caption{Middle base FEA: von Mises stress distribution under combined cable and jaw-transmitted loads}
    \label{fig:mid_fea}
\end{figure}

Overall, the FEA results indicate that both the jaw and middle base have ample structural capacity under the stated loading conditions. Local stress concentrations near cable holes and sharp edges were identified; applying small fillets, ensuring adequate material thickness around guide features, and standard deburring practices are recommended to mitigate these effects.
