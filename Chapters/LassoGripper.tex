...
\end{tabular}
\end{figure}
Its dual-core processor handles real-time sensor processing, execution of the grasping algorithms, and communication with external devices. The controller interfaces with a spool-mounted force sensor, motor encoders, and any auxiliary trackers used during experiments.

GPIO pins drive the retraction modules and friction-wheel controllers, while PWM outputs provide fine speed control for the propulsion motors. Wireless connectivity is used for logging, target-recognition offload, and remote monitoring via a host computer when required.
