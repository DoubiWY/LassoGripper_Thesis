...
\section{Development of Adaptive Grasping and Its Applications}
% Chapter summary moved to Chapters/RelatedWorks1.tex
...
Another novel technique employs a soft gripper that leverages a belt loop with an actuated adhesion mechanism. This mechanism provides bidirectional force, both radial and tangential, which enhances the effectiveness and stability of the grasping action. The minimal interaction force strategy ensures gentle handling, making it suitable for fragile objects \cite{chen2024soft}.
...
\caption{Pressurized actuation chamber and adaptive grasping \cite{hu2023dual}}
\end{minipage}
\begin{minipage}[t]{0.47\textwidth}
\centering
...
\begin{minipage}[t]{0.47\textwidth}
\centering
\includegraphics[width=\linewidth]{Figures/172.png}
\caption{Examples of enveloping grasps by the Velo gripper \cite{ciocarlie2014velo}}
\end{minipage}
\end{tabular}
\end{figure}
...
\caption{Illustration of the tactile sensor and pressure sensors integrated into the loop gripper \cite{hughes2020sensorization}}
\end{minipage}
\end{tabular}
\end{figure}
...
\subsection{Designing for the Physical Properties of Soft Grippers}
...
Wang et al. (2021) introduced a groundbreaking soft gripper using a self-sensing super-coiled polymer (SCP) actuator. This actuator is crafted from multi-strand sewing thread that undergoes a self-coiling process, enabling it to untwist when heated. The addition of a metal wire wrapped around the SCP enhances temperature measurement precision, forming a closed-loop control system for accurate gripper manipulation. This approach offers a foundation for customizing the physical properties of rope-like materials for actuator applications \cite{wang2021lightweight}.
...
Wang et al. (2017) explored the use of Shape Memory Alloy (SMA) in soft grippers, addressing the inherent softness and low actuation force of traditional systems. The study described an SMA-based soft gripper with variable stiffness, composed of three fingers, each with two hinges. The hinges can achieve a significant change in stiffness, allowing the gripper to adapt to various shapes and weights. This material modification approach shows promise for effective grasping applications \cite{wang2017shape}.
...
\subsection{Advancement of Grasp Planning Algorithms for Lasso Grippers}
...
\caption{A cable loop gripper prototype evaluated for grasping various types of chemistry glassware \cite{manes2024soft}}
\end{minipage}
...
\caption{String-shoot impact toward a wall \cite{Japan2023}}
\end{minipage}
\begin{minipage}[t]{0.47\textwidth}
\centering
...
\centering
\includegraphics[width=\linewidth]{Figures/sensors-22-04851-g014.png}
...
\label{fig:pneumatic_actuator_process}
\end{minipage}
% \qquad
...
\centering
\includegraphics[width=\linewidth]{Figures/chameleon.png}
\caption{Manipulating model of chameleon-like gripper \cite{mochiyama2015chameleon}.}
\label{fig:chameleon_manipulating_model}
\end{minipage}
% \qquad
% \begin{minipage}[t]{0.5\textwidth}
% \centering
...
\begin{minipage}[t]{0.9\textwidth}
\centering
\includegraphics[width=\linewidth]{Figures/fractal.png}
\caption{Fractal Hand prototype \cite{tisdale2023fractal}.}
\label{fig:fractal_hand_prototype}
\end{minipage}
% \qquad
% \begin{minipage}[t]{0.5\textwidth}
% \centering
...
Lasso Gripper is designed to address these gaps by incorporating a mechanism that can rapidly shoot and retract a string loop, enabling it to capture objects with a large initial capture range. Inspired by the traditional lasso used by cowboys and the uurga used by Mongolian herders, this mechanism leverages the structural advantages of a string loop to improve tolerance to object-position uncertainty and to create distributed contact during tightening. This design direction is particularly relevant for oversized, irregular, delicate, or moving targets where conventional fingertip placement is difficult.
...
The following chapters examine the design and working principles of Lasso Gripper, including its overall structure, string shooting-and-retraction mechanism, loop dynamics, control assumptions, and qualitative demonstrations. Point-cloud caging methods are reviewed as geometric motivation for loop placement, but the current prototype is treated as a preprogrammed mechanism demonstration rather than a fully autonomous visual grasping system. The experiments therefore compare representative grasping behaviors and failure modes rather than claiming complete autonomous grasp planning.
